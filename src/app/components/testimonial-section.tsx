...
const TestimonialCard = ({ testimonial }: { testimonial: Testimonial }) => (
  <div className="mb-8 sm:break-inside-avoid">
    <blockquote className="rounded-lg bg-surface-1 border border-hairline p-6 shadow-none sm:p-8 hover:bg-surface-2 hover:border-hairline-strong transition-colors duration-300">
      <div className="flex items-center gap-4">
        <Image
          alt={`Person de ${testimonial.author}`}
          src={testimonial.img}
          className="size-14 rounded-full object-cover border border-hairline"
        />
        <div>
          ...
          <p className="mt-0.5 text-lg font-medium text-ink">{testimonial.author}</p>
        </div>
      </div>
      <p className="mt-4 text-ink-muted text-sm leading-relaxed">{testimonial.text}</p>
    </blockquote>
  </div>
);
